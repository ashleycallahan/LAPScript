...
// @supportURL   https://github.com/ashleycallahan/LAPScript/issues
...
app-admin-manual-audits .review-mode-added-link-parent {
    display: flex;
    gap: 10px;
}
app-admin-manual-audits .review-mode-added-link-parent span {
    flex: 1;
}
app-admin-manual-audits .review-mode-added-link-parent span.margin-right {
    margin-right: 0px !important;
    order: 1;
}
app-admin-manual-audits .review-mode-added-link-parent ds-chip {
    order: 0;
}
    ...
    window.updateManualAudits = function() {
        if ($('app-admin-manual-audits').length > 0) {
            for (let i = 0; i < window.manualAudits.length; i++) {
                let thisAudit = $('app-admin-manual-audits table span').filter(function() {
                    return ($(this).text().trim() === window.manualAudits[i].title)
                });
                if (thisAudit.find('.review-mode-added-link').length === 0) {
                ...
                thisAudit.closest('td[colspan="2"]').removeAttr('colspan').next('td').attr('colspan', (thisAudit.closest('table').find('> thead > tr > th').length - 1));
            }
        }
        if ($('.review-mode-search').length === 0) {
    ...
    window.replaceAttachments = function() {
        ...
        if (typeof window.screens !== 'undefined') {
            if (filterByScreen.length > 0) {
                ...
                for (let i = 0; i < window.screens.length; i++) {
                    if (typeof window.screens[i].page !== 'undefined' && $('.review-mode-screen-select').find('option:contains(' + window.screens[i].page.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + ')').length === 0) {
