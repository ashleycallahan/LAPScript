...
(function() {
    ...
    window.addScreenImg = function() {
        for (var i = 0; i < window.screens.length; i++) {
            if (typeof window.screens[i].page !== 'undefined' && typeof window.screens[i].page.name !== 'undefined' && typeof window.screens[i].page.screenshot.src !== 'undefined') {
    ...
    window.getScreenImg = function(pageLink, url) {
        ...
        xhr.onload = function() {
            if (this.status === 200) {
                ...
                $(pageLink).after('<a href="' + blobURL + '" target="_blank" style="display:block;margin:10px 0 0 0;" class="review-mode-added"><img src="' + blobURL + '" style="max-width:200px"></a>');
            } else {}
        }
    ...
    window.replaceAttachments = function() {
        ...
        if (filterByScreen.length > 0 && filterByScreen.val() !== '') {
            if ($('app-manual-eval-findings-table').find('h3.review-mode-added').length === 0) {
                $('app-manual-eval-findings-table').prepend('<h3 class="review-mode-added">Page: ' + $('app-filter-keyword label:contains("Screen")').next('input').val() + '</h3>');
            }
        }
        else {
            $('app-manual-eval-findings-table').find('h3.review-mode-added').remove();
        }
